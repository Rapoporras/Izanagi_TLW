...
namespace KrillAudio.Krilloud.Editor
{
	...
	public sealed class KLAudioSourceEditor : Editor
	{
		...
		private void DrawHandlesLineType()
		{
			...
			// Draw handles
			EditorGUI.BeginChangeCheck();
			var fmh_323_59_638526837077212567 = Quaternion.identity; Vector3 targetStart = Handles.FreeMoveHandle(startPos, sizeStart, Vector3.zero, Handles.DotHandleCap);
			var fmh_324_55_638526837077247707 = Quaternion.identity; Vector3 targetEnd = Handles.FreeMoveHandle(endPos, sizeEnd, Vector3.zero, Handles.DotHandleCap);
			if (EditorGUI.EndChangeCheck())
			{
